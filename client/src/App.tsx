import { useState } from "react";
import "./App.css";

type Card = string;
type GameStatus = "idle" | "playing" | "won" | "lost" | "tie";

type ApiState = {
  id: string;
  status: GameStatus;
  reveal: boolean;
  dealer: { visible: Card[]; count?: number };
  player: { cards: Card[]; count: number };
  playerGold: number;
  dealerGold: number;
  currentBet: number; // server-locked bet during an active round
};

export default function App() {
  // server state
  const [gameId, setGameId] = useState<string | null>(null);
  const [status, setStatus] = useState<GameStatus>("idle");
  const [reveal, setReveal] = useState(false);
  const [playerCards, setPlayerCards] = useState<Card[]>([]);
  const [dealerVisible, setDealerVisible] = useState<Card[]>([]);
  const [playerCount, setPlayerCount] = useState(0);
  const [dealerCount, setDealerCount] = useState<number | undefined>(undefined);
  const [playerGold, setPlayerGold] = useState(1000);
  const [dealerGold, setDealerGold] = useState(1000);
  const [currentBet, setCurrentBet] = useState(0); // from server after DEAL

  // local UI state (choosing chips before deal)
  const [bet, setBet] = useState(0);
  const chips = [10, 50, 100, 500];

  function apply(r: ApiState) {
    setGameId(r.id);
    setStatus(r.status);
    setReveal(r.reveal);
    setPlayerCards(r.player.cards);
    setDealerVisible(r.dealer.visible);
    setPlayerCount(r.player.count);
    setDealerCount(r.dealer.count);
    setPlayerGold(r.playerGold);
    setDealerGold(r.dealerGold);
    setCurrentBet(r.currentBet ?? 0);

    // when a round begins, clear the choosing UI; when it ends, no locked bet
    if (r.status === "playing") setBet(0);
    else setCurrentBet(0);
  }

  // preview balances:
  // - before deal: subtract the chosen bet
  // - during play: subtract the server-locked currentBet
  const subtract = status === "playing" ? currentBet : bet;
  const displayPlayerGold = Math.max(0, playerGold - subtract);
  const displayDealerGold = Math.max(0, dealerGold - subtract);

  function addChip(v: number) {
    if (status === "playing") return;                   // cannot change mid-round
    setBet((b) => Math.min(b + v, playerGold));         // cap by bankroll
  }
  function clearBet() {
    if (status === "playing") return;
    setBet(0);
  }

  async function onDeal() {
    if (status === "playing") return;
    if (bet === 0 || bet > playerGold) return;

    const resp = await fetch("/api/games/start", {
      method: "POST",
      headers: { "Content-Type": "application/json" },
      body: JSON.stringify({ id: gameId ?? undefined, bet }),
    });
    if (!resp.ok) {
      const err = await resp.json().catch(() => ({}));
      console.error("DEAL error:", err);
      return;
    }
    const r: ApiState = await resp.json();
    apply(r); // switches to playing
  }

  async function onHit() {
    if (!gameId || status !== "playing") return;
    const resp = await fetch(`/api/games/${gameId}/hit`, { method: "POST" });
    const r: ApiState = await resp.json();
    apply(r);
  }

  async function onStay() {
    if (!gameId || status !== "playing") return;
    const resp = await fetch(`/api/games/${gameId}/stay`, { method: "POST" });
    const r: ApiState = await resp.json();
    apply(r);
  }

  return (
    <div className="app">
      <h1>Blackjack</h1>

      {/* Bankroll / Bet */}
      <div className="panel">
        <div className="bank">
          <div>Your Gold: <strong>{displayPlayerGold}</strong></div>
          <div>Dealer’s Gold: <strong>{displayDealerGold}</strong></div>
        </div>

        <div className="bet-row">
          <div>Bet: <strong>{status === "playing" ? currentBet : bet}</strong></div>
          <div className="chips">
            {chips.map((c) => (
              <button
                key={c}
                onClick={() => addChip(c)}
                disabled={status === "playing" || bet + c > playerGold}
              >
                {c}
              </button>
            ))}
            <button onClick={clearBet} disabled={status === "playing"}>Clear</button>
            <button
              className="deal"
              onClick={onDeal}
              disabled={status === "playing" || bet === 0 || bet > playerGold}
            >
              DEAL
            </button>
          </div>
        </div>
      </div>

      {/* Table */}
      <div className="table">
        <section>
          <h2>Dealer {reveal && dealerCount !== undefined ? `(${dealerCount})` : ""}</h2>
          <div className="cards">
            {dealerVisible.map((c, i) => (
              <img
                key={i}
                src={reveal ? `/cards/${c}.png` : i === 0 ? `/cards/${c}.png` : "/cards/BACK.png"}
                alt={c}
              />
            ))}
            {/* if server sends only the up card while hidden one is separate */}
            {!reveal && dealerVisible.length === 1 && (
              <img src="/cards/BACK.png" alt="Hidden" />
            )}
          </div>
        </section>

        <section>
          <h2>You ({playerCount})</h2>
          <div className="cards">
            {playerCards.map((c, i) => (
              <img key={i} src={`/cards/${c}.png`} alt={c} />
            ))}
          </div>
        </section>
      </div>

      {/* Actions */}
      <div className="actions">
        <button onClick={onHit} disabled={status !== "playing"}>Hit</button>
        <button onClick={onStay} disabled={status !== "playing"}>Stay</button>
        {status !== "playing" && status !== "idle" && (
          <div className={`result ${status}`}>Result: {status}</div>
        )}
      </div>
    </div>
  );
}

